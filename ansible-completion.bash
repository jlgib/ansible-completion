...
_ansible_complete_host() {
    ...
    local hostnames=$(ansible ${inventory_file:+-i "$inventory_file"} all --list-hosts 2> /dev/null \
        && [ -e "$inventory_file" ] \
        && [ -d "$inventory_file" -o ! -x "$inventory_file" ] \
        && grep $grep_opts '\[.*\]' "$inventory_file" | tr -d [] | cut -d: -f1)

    grep_opts="$grep_opts -oP"
    local arg_hostnames=$(grep $grep_opts 'hostname=[^\n ]*' "$inventory_file" 2> /dev/null | cut -d= -f2)

    hosts="$hostnames $arg_hostnames"
